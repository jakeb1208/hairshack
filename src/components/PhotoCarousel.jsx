...
const PLACEHOLDERS = [
  { bg: 'linear-gradient(135deg, #fce4ec 0%, #f8bbd0 100%)', icon: '✂', label: 'Photo 1' },
  { bg: 'linear-gradient(135deg, #ffebee 0%, #ffcdd2 100%)', icon: '💈', label: 'Photo 2' },
  { bg: 'linear-gradient(135deg, #fce4ec 0%, #ef9a9a 100%)', icon: '✂', label: 'Photo 3' },
  { bg: 'linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%)', icon: '💈', label: 'Photo 4' },
  { bg: 'linear-gradient(135deg, #fbe9e7 0%, #ffccbc 100%)', icon: '✂', label: 'Photo 5' },
  { bg: 'linear-gradient(135deg, #fce4ec 0%, #f48fb1 100%)', icon: '💈', label: 'Photo 6' },
]
...
export default function PhotoCarousel({ photos = {} }) {
  ...
  const slides = PLACEHOLDERS.map((p, i) => ({
    ...p,
    src: photos[`photo.${i}`] || '',
  }))
  ...
  const onDragStart = (e) => { setDragging(true); setStartX(e.clientX ?? e.touches?.[0]?.clientX) }
  const onDragEnd = (e) => {
    if (!dragging) return
    setDragging(false)
  ...
  const getSlides = () =>
    Array.from({ length: VISIBLE }, (_, i) => slides[(current + i) % total])
  ...
  return (
    <section style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            {getSlides().map((slide, i) => (
              <div
                ...
                style={{
                  ...
                  boxShadow: i === 1 ? 'var(--shadow-glow-strong)' : '0 4px 20px rgba(0,0,0,0.07)',
                  transform: i === 1 ? 'scale(1.04)' : 'scale(1)',
                  transition: 'transform 0.4s ease, box-shadow 0.4s ease',
                  cursor: 'grab',
                  background: slide.src ? '#000' : slide.bg,
                  position: 'relative',
                }}
              >
                {slide.src ? (
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginTop: '1.75rem' }}>
          {slides.map((_, i) => (
            <button
              key={i}
              onClick={() => goTo(i)}
